...
import {
    ...
    getItem,
    toggleFavorite,
    checkIfFavoriteIsTrue,
} from '../utils/sessionStorageUtil'

const Filters = () => {
    const [localCampus, setLocalCampus] = useState<string>(
        () => getItem('campus') || 'Hvilket campus?'
    )
    ...
    const [localRoomType, setLocalRoomType] = useState<string>(
        () => getItem('roomType') || 'Romtype?'
    )
    const [localFavorites, setFavoriteChoice] = useState<boolean>(() =>
        checkIfFavoriteIsTrue('favorite')
    )
