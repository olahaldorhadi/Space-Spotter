...
const fetchRoomDetails = async () => {
    const response = await fetch(
        '/ntnu/ws/room/2.0/allrooms.php?year=' +
            currentYear +
            '&week=' +
            currentWeek +
            '&fulldetails=1',
...
function useFilteredRooms(): UseFilteredRoomsReturnType {
    const { data, error, isLoading } = useQuery<DataResponse, CustomError>(
        'roomDetails',
        fetchRoomDetails
    )

    const { campus, size, roomType, favorites } =
        useContext(FilterContext) || {}
    const areaNameFilter = (room: Room) => room.areaname === campus
    const sizeFilter = (room: Room) =>
        parseInt(room.size) >= parseInt(size || '0')
    const typeFilter = (room: Room) => room.type === roomType
    const favoritesFilter = favorites
        ? (room: Room) => (getList() || []).includes(room.name)
        : () => true
    ...
    const filteredRooms = data?.data
        ...
        .map((room, index) => ({
            ...
            size: room.size,
            bookable:
                room.student_booking == '1' ? 'Kan bookes' : 'Kan ikke bookes',
            type: room.type,
